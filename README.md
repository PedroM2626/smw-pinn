...
**E2 - real-data identification and the misspecification ceiling.** Fitting the *same* extended model to genuine WRAM gameplay ($6{,}249$ train / $1{,}332$ test within-episode windows) lowers test open-loop rollout error more than the six-constant model did ($x$-channel MSE $5.74\to4.58\ \text{px}^2$, $-20\%$; $v_x$ $40.8\to30.0$, $v_y$ $831\to714$) - the added coast friction and ground-contact reset *do* close part of the gap, pushing the identified prediction below the six-constant fit's $4.83$. Yet the recovered point estimates of several constants still drift far from their reverse-engineered values (e.g. $\hat a_{\text{walk}}\approx0.09$, $\hat\sigma\approx21$, while $\hat c\approx0.44$ sits close to a plausible friction). This is the honest limit of the *structural* model, not the estimator: the map still excludes collision response, slopes and wall checks, so on real data the free constants trade off against one another. Point estimates are only as trustworthy as the forward model's validity; prediction still improves, and the richer physics makes it improve more.
