...
\subsection{Discrete Kinematic Consistency Formulation}
...
\begin{theorem}[Discrete Kinematic Consistency]
Let $s_t = (X_t, Y_t, v_{x, t}, v_{y, t})$ be the true state and $a_t$ the Joypad input. Any forward transition model $f_\theta(s_t, a_t)$ that predicts $\hat{s}_{t+1}$ satisfies strict discrete kinematic consistency if and only if the analytical kinematic residual:
\begin{equation}
...
A critical theoretical finding of this study is the failure of the Soft PINN ($\text{MSE} = 29.23$ vs $17.47$ for pure MLP). In discrete dynamical systems with non-smooth friction and contact boundaries, Lagrangian regularization introduces gradient conflicts between the supervised loss $\nabla_\theta \mathcal{L}_{\text{data}}$ and the physical penalty $\nabla_\theta \mathcal{R}_{\text{kin}}$. Attempting to enforce exact kinematic integration identities purely through loss penalties leads to Pareto suboptimality. In contrast, embedding the Eulerian kinematic identity directly into the computational graph as a hard structural prior completely eliminates optimization tension.

\section{Conclusion}
This work demonstrates that embedding discrete kinematic consistency constraints as hard structural physical priors fundamentally outperforms statistical black boxes and soft penalty regularization in discrete world modeling. The resulting Hard Residual PINN achieves zero kinematic drift, high sample efficiency, and robust closed-loop planning capability on authentic console emulation.
